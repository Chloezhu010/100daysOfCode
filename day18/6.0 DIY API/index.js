...
app.get("/filter", (req, res)=> {
  ...
  const filteredJokes = jokes.filter((joke)=> joke.jokeType === type); //filter the jokes array for jokes that match the type query parameter
...
app.put("/jokes/:id", (req, res)=>{
  ...
  // diff between find() and findIndex()
  // find() returns the first element that satisfies the condition
  // findIndex() returns the index of the first element that satisfies the condition
  const searchIndex = jokes.findIndex((joke)=> joke.id === id); //find the index of the joke we want to replace
  jokes[searchIndex] = replaceJoke; //replace the joke at the index with the new joke
...
app.delete("/jokes/:id", (req, res)=>{
  ...
  const searchIndex = jokes.findIndex((joke)=> joke.id === id);
  if (searchIndex > -1){
    jokes.splice(searchIndex, 1); // remove the joke at the index
    res.sendStatus(200);
} else {
